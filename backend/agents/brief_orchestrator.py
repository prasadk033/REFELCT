...
from db import SessionLocal, Source, Brief, BriefSource, Card, ProcessingJob, Project, SiteAnalysisCache
...
def run_brief_pipeline(project_id: str, source_ids: List[str], job_id: str, user_id: str = None):
    ...
    try:
        ...
        # ── Step 6.5: OSM Site Analysis Generation ────────────────────────
        if project.location:
            try:
                # 1. Check cache
                ...
                if cached_osm:
                    logger.info(f"[{project_id}] Using cached OSM Site Analysis.")
                    print(f"[REFLECT] 🌍 Loaded cached OpenStreetMap site analysis for '{project.location}'")
                    structured_analysis = cached_osm.structured_analysis
                else:
                    logger.info(f"[{project_id}] Running OSM Site Analysis for location: {project.location}")
                    print(f"[REFLECT] 🌍 Fetching live OpenStreetMap data for '{project.location}'...")
                    ...
                    if geo_res:
                        ...
                        if raw_osm.get("elements"):
                            ...
                            db.add(new_cache)
                            db.commit()
                            print(f"[REFLECT] 🌍 OpenStreetMap data successfully analyzed and cached.")
                        else:
                            logger.warning(f"[{project_id}] OSM Overpass returned no elements.")
                            print(f"[REFLECT] ⚠️ OpenStreetMap returned no elements for this location.")
                    else:
                        logger.warning(f"[{project_id}] OSM Geocoding failed for {project.location}.")
                        print(f"[REFLECT] ⚠️ OpenStreetMap could not resolve the site location.")

                if structured_analysis:
                    master_card_data = osm_service.generate_master_card(structured_analysis, project_id, brief_id)
                    master_card = Card(
                        id=master_card_data["id"],
                        project_id=project_id,
                        brief_id=brief_id,
                        card_type=master_card_data["card_type"],
                        title=master_card_data["title"],
                        content=master_card_data["content"],
                        evidence=master_card_data["evidence"],
                        section=master_card_data["section"],
                        version=new_version,
                        created_by=master_card_data["created_by"],
                        status=master_card_data["status"]
                    )
                    db.add(master_card)
                    total_new_cards += 1
                    logger.info(f"[{project_id}] Generated Master OSM Site Context Card.")

            except Exception as e:
                logger.error(f"[{project_id}] OSM Site Analysis failed, skipping gracefully: {e}")
                print(f"[REFLECT] ⚠️ OpenStreetMap site analysis failed, skipping.")
